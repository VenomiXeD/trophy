...
function doTick(tickable: Tickable, dt: number): void {
...
class ClientUpdateManager extends UpdateManager {
    ...
    constructor(trophyInstance: Trophy) {
        ...
        this.rService.Heartbeat.Connect(dt=>{
            this.tickables.forEach(t=>doTick(t,dt));
        });

        this.rService.BindToRenderStep("TROPHYRENDER",math.huge,dt=>{
            this.rendertickables.forEach(t=>doTick(t,dt));
        });
    }
...
export {
    UpdateManager, ClientUpdateManager, Tickable
}